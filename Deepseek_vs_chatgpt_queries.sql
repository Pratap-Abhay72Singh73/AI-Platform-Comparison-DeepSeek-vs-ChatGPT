...
select * from deepseek_vs_chatgpt limit 1;
...
WITH daily AS (
  ...
  GROUP BY AI_Platform, day
)
SELECT
  AI_Platform,
  concat(round(SUM(daily_active_users)/1000000,2),'M') AS Total_active_users
FROM daily
...
SELECT 
  ai_platform, 
  month_num, 
  concat(ROUND(AVG(daily_churn_rate), 2)*100,'%')AS monthly_churn_rate
FROM deepseek_vs_chatgpt
...
select ai_model_version,weekday,concat(round(avg(daily_churn_rate),2)*100,'%') as Churned_pct
from deepseek_vs_chatgpt
...
select ai_platform,concat(round(avg(retention_rate),2)*100,'%') as Retention_Rate
from deepseek_vs_chatgpt
group by ai_platform;
...
select Device_Type,max(user_return_frequency) as max_user_return_freq,min(user_return_frequency) as min_user_return_freq,round(avg(user_return_frequency),2) as avg_user_return_freq
from deepseek_vs_chatgpt
